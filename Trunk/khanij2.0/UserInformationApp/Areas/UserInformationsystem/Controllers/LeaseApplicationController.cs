        ...
        public IActionResult PreferredBidder(PreferredBidder model, List<string> ParticipantRadio, List<string> GovRadio, List<string> ParticipantName, string btnValue)
        {
            ...
            else if (Convert.ToInt16(model.DistrictId) == 0)
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    LeaseModel.PeriodFrom = model.PeriodFrom;
                    LeaseModel.PeriodTo = model.PeriodTo;
                    LeaseModel.BidStart = model.BidStart;
                    LeaseModel.BidFinal = model.BidFinal;
                    LeaseModel.BlockName = model.BlockName;
                    LeaseModel.BidUnitId = model.BidUnitId;
                    LeaseModel.BidSheetFileName = model.BidSheetFileName;
                    LeaseModel.BidSheetFilePath = model.BidSheetFilePath;
                    LeaseModel.AreaInHect = model.AreaInHect;
                    ...
                    LeaseModel.Particpantlist = model.ParticipatesTable;
            ...
            DateTime? ChallanDate, string PMode, string NetBanking_mode, List<int> DocId)
        {
            ...
            try
            {
                ...
                if (objMessage.Satus == "1" && model.SpMode == "DGMForward")
                {
                    try
                    {
                        UserInformationApp.Areas.UserInformationsystem.Controllers.DSCResponseController objDSCResponse = new DSCResponseController(_objIticketmodel, hostingEnvironment, configuration);
