...
namespace EstablishmentApp.Areas.WorkFlow.Models.WorkFlow
{
    public class WorkFlowModel : IWorkFlowModel
    {
        ...
        public List<WorkFlowDropDown> BindDropDownSetAuthority(WorkFlowDropDown objWorkFlow)
        {
            ...
            try
            {
                listLeave = JsonConvert.DeserializeObject<List<WorkFlowDropDown>>(_objIHttpWebClients.PostRequest("LeaveManagement/BindDropDownSetAuthority", JsonConvert.SerializeObject(objWorkFlow)));
                return listLeave;
            }
            catch (Exception)
            {
        ...
        {
            WorkFlowEF _objWorkFlow = WorkFlowEF.GetInstance();
            try
            {
